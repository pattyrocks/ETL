...
def create_credits():
    ...
    with ThreadPoolExecutor(max_workers=MAX_API_WORKERS) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if processed_movies_count % (total_movies_to_process // 10 or 1) == 0 or processed_movies_count == total_movies_to_process:
                ...
                log_and_print(f"Progress: {progress_percent:.1f}% | Processed {processed_movies_count}/{total_movies_to_process} movies | Fetched {total_crew_members_count} crew members | Elapsed API fetch time: {elapsed_api_time:.2f}s")
    ...
    if not all_crew_data_flat:
        log_and_print("No crew data retrieved. Exiting.")
        con.close()
        return
    ...
    try:
        ...
        log_and_print("Created/verified 'movie_crew' table schema.")
        ...
        if missing_df_columns:
            log_and_print(f"Warning: The following columns are defined for insertion but missing from the DataFrame: {missing_df_columns}. They will be inserted as NULL.", level='warning')
        ...
        if DB_INSERT_BATCH_SIZE is None or len(crew_df_reordered) <= DB_INSERT_BATCH_SIZE:
            ...
            log_and_print(f"Inserted all {len(crew_df_reordered)} rows into 'movie_crew' in {time.time() - t0:.2f}s.")
        else:
