...
def fetch_tv_show_crew(tv_id):
    for attempt in range(MAX_RETRIES):
        try:
            ...
            log_and_print(f"Error fetching crew for tv_id {tv_id}: {e}", level='error')
            return []
...
def update_tv_show_crew(con):
    log_and_print("=" * 60)
    log_and_print("STARTING TV SHOW CREW UPDATE")
    log_and_print("=" * 60)
    ...
    processed_ids = load_checkpoint('tv_crew_checkpoint.pkl')
    ...
    if total_tv_to_process == 0:
        ...
        check_and_remove_duplicates(con, 'tv_show_crew', TV_CREW_PARTITION_COLS, TV_CREW_SELECT_COLS)
        return

    if DRY_RUN:
        log_and_print(f"[DRY RUN] Would fetch crew for {total_tv_to_process} TV shows")
        return
    ...
    with ThreadPoolExecutor(max_workers=MAX_API_WORKERS) as executor:
        future_to_tv_id = {
            executor.submit(fetch_tv_show_crew, tv_id): tv_id
        ...
        for future in as_completed(future_to_tv_id):
            ...
            try:
                ...
                if processed_tv_count % API_BATCH_SIZE == 0:
                    ...
                    log_and_print(f"TV Crew Progress: {processed_tv_count}/{total_tv_to_process} ({percent:.2f}%)")
                    save_checkpoint(processed_ids, 'tv_crew_checkpoint.pkl')
            except Exception as e:
                log_and_print(f"Error processing tv_id {tv_id}: {e}", level='error')
                skipped_ids.append(tv_id)

    elapsed = time.time() - start_time
    log_and_print(f'Finished TV crew API retrieval in {elapsed:.2f}s')
    log_and_print(f'Total TV crew members fetched: {len(all_crew_data_flat)}')

    if not all_crew_data_flat:
        log_and_print("No TV crew data to insert.")
        check_and_remove_duplicates(con, 'tv_show_crew', TV_CREW_PARTITION_COLS, TV_CREW_SELECT_COLS)
        log_skipped_ids(skipped_ids, 'tv_crew_skipped_ids.log')
        return
    ...
    for i in range(num_batches):
        ...
        try:
            ...
            con.execute(f"INSERT INTO tv_show_crew ({tv_columns}) SELECT {tv_columns} FROM batch_df_view;")
            log_and_print(f"TV Crew Batch {i+1}/{num_batches} inserted ({len(batch_df)} rows)")
        except Exception as e:
            log_and_print(f"Error inserting TV crew batch: {e}", level='error')
    ...
    log_and_print(f'TV show crew update complete in {total_elapsed:.2f}s')
